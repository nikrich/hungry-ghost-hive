...
import { IntermediateAgent, type IntermediateContext } from './intermediate.js';
...
describe('IntermediateAgent', () => {
  let db: Database;
  let provider: MockLLMProvider;
  let agentRow: AgentRow;
  let teamId: string;
  let context: IntermediateContext;
  ...
  describe('getSystemPrompt', () => {
    it('should include team information', () => {
      ...
      expect(prompt).toContain('Intermediate Developer');
    ...
    it('should show "No team assigned" when team is null', () => {
      ...
      const agent = new IntermediateAgent(noTeamContext);
      const prompt = agent.getSystemPrompt();
    ...
    it('should mention complexity range 4-5 points', () => {
      const agent = new IntermediateAgent(context);
      const prompt = agent.getSystemPrompt();

      expect(prompt).toContain('4-5 points');
    });

    it('should include escalation guidance', () => {
      const agent = new IntermediateAgent(context);
      const prompt = agent.getSystemPrompt();

      expect(prompt).toContain('Request help from Senior');
  ...
  describe('execute', () => {
    ...
    it('should set branch name when implementing story', async () => {
      db.run(
        `INSERT INTO stories (id, team_id, title, description, status) VALUES (?, ?, ?, ?, ?)`,
        ['story-1', teamId, 'Implement Feature', 'Description', 'planned']
      );
  ...
  describe('retry and escalation', () => {
    it('should escalate to senior after max retries', async () => {
      // Create senior agent
      db.run(`INSERT INTO agents (id, type, team_id, status) VALUES (?, ?, ?, ?)`, [
        'senior-1',
        'senior',
      ...
      const storyContext = {
        ...context,
        storyId: 'story-1',
        config: { ...context.config, maxRetries: 1 },
      };
      const agent = new IntermediateAgent(storyContext);
    ...
    it('should retry before escalating', async () => {
      db.run(`INSERT INTO agents (id, type, team_id, status) VALUES (?, ?, ?, ?)`, [
        'senior-1',
        'senior',
        teamId,
      ...
      const storyContext = {
        ...context,
        storyId: 'story-1',
        config: { ...context.config, maxRetries: 3 },
      };
      const agent = new IntermediateAgent(storyContext);
      ...
      const logs = queryAll<{ event_type: string; message: string }>(
        db,
        'SELECT event_type, message FROM event_logs WHERE event_type = ?',
        ['STORY_PROGRESS_UPDATE']
      );

      const retryLog = logs.find(l => l.message.includes('Retry'));
