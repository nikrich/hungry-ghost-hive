  ...
  .action(async (options: { json?: boolean }) => {
    await withReadOnlyHiveContext(async ({ db }) => {
      ...
      for (const team of teams) {
        const agents = getAgentsByTeam(db.provider, team.id);
        const stories = getStoriesByTeam(db.provider, team.id);
        const storyPoints = getStoryPointsByTeam(db.provider, team.id);
...
teamsCommand
  ...
  .action(async (name: string) => {
    await withReadOnlyHiveContext(async ({ db }) => {
      const team = getTeamByName(db.provider, name);
...
teamsCommand
  ...
  .action(async (name: string, options: { force?: boolean }) => {
    await withHiveContext(async ({ db }) => {
      ...
      deleteTeam(db.provider, team.id);
