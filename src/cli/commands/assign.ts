  ...
  .action(async (options: { dryRun?: boolean }) => {
    ...
    await withHiveContext(async ({ root, paths, db }) => {
      ...
      try {
        ...
        const plannedStories = getPlannedStories(db.provider);
        let godmodeActive = false;
        for (const story of plannedStories) {
          if (story.requirement_id) {
            const requirement = getRequirementById(db.provider, story.requirement_id);
        ...
        if (options.dryRun) {
          ...
          for (const [teamId, stories] of storiesByTeam) {
            const team = getTeamById(db.provider, teamId);
            if (!team) continue;
