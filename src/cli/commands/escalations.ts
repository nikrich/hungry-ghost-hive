  ...
  .action(async (options: { all?: boolean; json?: boolean }) => {
    await withReadOnlyHiveContext(async ({ db }) => {
      const escalations = options.all
        ? getAllEscalations(db.provider)
        : getPendingEscalations(db.provider);
...
escalationsCommand
  ...
  .action(async (id: string) => {
    await withReadOnlyHiveContext(async ({ db }) => {
      const escalation = getEscalationById(db.provider, id);
      if (!escalation) {
...
escalationsCommand
  ...
  .action(async (id: string, options: { message: string }) => {
    await withHiveContext(async ({ db }) => {
      ...
      const resolved = resolveEscalation(db.provider, id, options.message);
...
escalationsCommand
  ...
  .action(async (id: string) => {
    await withHiveContext(async ({ db }) => {
      ...
      acknowledgeEscalation(db.provider, id);
