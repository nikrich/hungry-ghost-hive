...
export async function checkFeatureTestResult(ctx: ManagerCheckContext): Promise<void> {
  ...
  const candidates = await ctx.withDb(async db => {
    const featureTestAgents = getAgentsByType(db.provider, 'feature_test');
    ...
    for (const agent of featureTestAgents) {
      const recentLogs = db.provider.queryAll<AgentLogRow>(
        `
      ...
      const alreadyProcessed = db.provider.queryAll<AgentLogRow>(
        `
        SELECT * FROM agent_logs
      ...
      const spawnLog = db.provider.queryAll<AgentLogRow>(
        `
        SELECT * FROM agent_logs
        WHERE agent_id = ?
      ...
      const requirement = getRequirementById(db.provider, requirementId);
      if (!requirement) {
        verboseLogCtx(
          ctx,
...
Co-Authored-By: Feature Test Agent <noreply@hive>`;
    ...
    await ctx.withDb(async db => {
      updateRequirement(db.provider, requirementId, { status: 'sign_off_passed' });
    ...
    await ctx.withDb(async db => {
      updateRequirement(db.provider, requirementId, { status: 'sign_off' });
      createLog(db.provider, {
        agentId: 'manager',
...
async function handleTestFailure(
  ...
  await ctx.withDb(async db => {
    updateRequirement(db.provider, requirementId, { status: 'sign_off_failed' });
    createLog(db.provider, {
      agentId: 'manager',
      eventType: 'FEATURE_SIGN_OFF_FAILED',
