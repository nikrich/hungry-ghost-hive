...
export async function handleEscalationAndNudge(
  ...
  const hasRecentEscalation =
    ...
    (agentId !== null &&
      (await ctx.withDb(
        async db =>
          getRecentEscalationsForAgent(db.provider, agentId, RECENT_ESCALATION_LOOKBACK_MINUTES)
            .length > 0
      )));
  ...
  if (waitingInfo.needsHuman && !hasRecentEscalation) {
    ...
    const resolvedCount = await ctx.withDb(async db => {
      const activeEscalations = agentId ? getActiveEscalationsForAgent(db.provider, agentId) : [];
