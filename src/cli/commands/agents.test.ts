...
describe('agents command', () => {
  ...
  describe('logs subcommand', () => {
    ...
    it('should have --limit option', () => {
      const logsCmd = agentsCommand.commands.find(cmd => cmd.name() === 'list');
      const limitOpt = logsCmd?.options.find(opt => opt.long === '--limit');
      expect(limitOpt).toBeUndefined();
      const logsCmd2 = agentsCommand.commands.find(cmd => cmd.name() === 'logs');
      const limitOpt2 = logsCmd2?.options.find(opt => opt.long === '--limit');
      expect(limitOpt2).toBeDefined();
    });
