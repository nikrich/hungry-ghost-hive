...
agentsCommand
  ...
  .action(async (options: { active?: boolean; json?: boolean }) => {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `${'ID'.padEnd(25)} ${'Type'.padEnd(12)} ${'Team'.padEnd(15)} ${'Status'.padEnd(12)} ${'Current Story'}`
        )
      );
      ...
      for (const agent of agents) {
        ...
        console.log(
          `${chalk.cyan(agent.id.padEnd(25))} ${agent.type.padEnd(12)} ${team.padEnd(15)} ${statusColor(agent.status).padEnd(12)} ${story}`
        );
      }
      console.log();
    } finally {
