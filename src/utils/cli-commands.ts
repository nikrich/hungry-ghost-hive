...
export function getAvailableCommands(cliTool: CLITool): {
  ...
  switch (cliTool) {
    ...
    case 'codex':
      ...
      throw new Error('Codex CLI commands not yet implemented');
    case 'gemini':
      // TODO: Implement in future story
      throw new Error('Gemini CLI commands not yet implemented');
    default:
      return buildClaudeCommands(); // Default fallback
  }
}
 ...
 */
export function formatCommandForAgent(command: string, cliTool: CLITool): string {
...
${formatCommandForAgent(commands.getMyStories(sessionName, true), cliTool)}
