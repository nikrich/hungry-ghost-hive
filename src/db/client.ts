...
function runMigrations(db: SqlJsDatabase): void {
  ...
  // Migration 007: Add indexes for frequently-queried columns
  const result007 = db.exec("SELECT name FROM migrations WHERE name = '007-add-query-indexes.sql'");
  ...
  if (!migration007Applied) {
    ...
    db.run("CREATE INDEX IF NOT EXISTS idx_pull_requests_team_id_status ON pull_requests(team_id, status)");
    ...
    db.run("INSERT INTO migrations (name) VALUES ('007-add-query-indexes.sql')");
  }
}
