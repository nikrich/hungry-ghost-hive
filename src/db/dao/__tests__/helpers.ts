...
/**
 * Creates an in-memory sql.js database with the full schema (all migrations applied).
 */
export async function createTestDb(): Promise<Database> {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS stories (
      ...
      status TEXT DEFAULT 'draft' CHECK (status IN (
        'draft', 'estimated', 'planned', 'in_progress', 'review', 'qa', 'qa_failed', 'pr_submitted', 'merged'
      )),
  ...
  db.run('CREATE INDEX IF NOT EXISTS idx_agent_logs_agent ON agent_logs(agent_id)');
  db.run('CREATE INDEX IF NOT EXISTS idx_agent_logs_story ON agent_logs(story_id)');
  db.run('CREATE INDEX IF NOT EXISTS idx_agent_logs_timestamp ON agent_logs(timestamp)');
