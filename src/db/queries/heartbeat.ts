 ...
 */
export function updateAgentHeartbeat(db: Database.Database, agentId: string): void {
...
export function getStaleAgents(
  ...
  return queryAll<{
    ...
    last_seen: string | null;
    seconds_since_heartbeat: number;
  }>(db, query, [timeoutSeconds, timeoutSeconds]);
}
...
export function isAgentHeartbeatCurrent(
  ...
  const row = queryOne<{ is_current: number }>(db, query, [timeoutSeconds, agentId]);
