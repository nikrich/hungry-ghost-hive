 ...
 *
 * TODO: Validate patterns against actual Codex CLI output
 */
...
const CODEX_STATE_INDICATORS: StateIndicator[] = [
  ...
  { state: AgentState.TOOL_RUNNING, patterns: [/running|executing/i, /tool:/i], priority: 100 },
  ...
  { state: AgentState.IDLE_AT_PROMPT, patterns: [/^>\s*$/m, /^codex>/i, /ready/i], priority: 40 },
];

export class CodexStateDetector implements StateDetector {
  detectState(output: string): StateDetectionResult {
    const sorted = [...CODEX_STATE_INDICATORS].sort((a, b) => b.priority - a.priority);
  ...
  getStateDescription(state: AgentState): string {
    return `Codex: ${state.replace(/_/g, ' ')}`;
  }
