...
export class Scheduler {
   ...
   */
  previewAssignments(): { assigned: number; assignments: Array<{ storyId: string; agentType: string }>; errors: string[] } {
    const plannedStories = getPlannedStories(this.db);
    const errors: string[] = [];
    const assignments: Array<{ storyId: string; agentType: string }> = [];
    let assigned = 0;
    ...
    for (const [teamId, stories] of storiesByTeam) {
      ...
      if (!senior) {
        ...
        senior = { type: 'senior' } as AgentRow;
      }
      ...
      for (const story of stories) {
        ...
        const complexity = story.complexity_score || 5;
        let targetAgentType: AgentRow['type'] | undefined;
