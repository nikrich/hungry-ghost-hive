 ...
 */
export function getContextFileName(cliTool) {
    switch (cliTool) {
        case 'claude-code':
            return 'CLAUDE.md';
        case 'codex':
 ...
 */
export function getContextFilePath(repoPath, cliTool) {
    return `${repoPath}/${getContextFileName(cliTool)}`;
}
/**
 * Check if a context file exists in a repository
 */
export function contextFileExists(repoPath, cliTool) {
 ...
 */
export function generateContextFile(options) {
 ...
 */
function mergeContextFileContent(existingContent, newContent) {
