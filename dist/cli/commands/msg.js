...
msgCommand
    ...
    .action(async (toSession, message, options) => {
...
msgCommand
    ...
    try {
      ...
      `;
        ...
        const messages = queryAll(db.db, query, [targetSession]);
...
msgCommand
    ...
    const db = await getDatabase(paths.hiveDir);
    try {
        const msg = queryOne(db.db, 'SELECT * FROM messages WHERE id = ?', [msgId]);
        if (!msg) {
